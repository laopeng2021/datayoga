...
export default class FeDateBlockProperties extends BlockProperties {
  ...
  onColumnChanged(newVal: string, oldVal: string) {
    ...
    if (!this.isDirtyColumnNames) {
      ...
      const columnNameParts = newVal.split(".");
      const columnName = columnNameParts[columnNameParts.length - 1];
      this.local.append_day_column = // eslint-disable-line
        columnName.replace(" ", "_").toLowerCase() + "_dt_day";
    }
  }
}
</script>
